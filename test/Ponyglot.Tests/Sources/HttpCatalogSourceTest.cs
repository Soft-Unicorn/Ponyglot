...
public class HttpCatalogSourceTest
{
    ...
    [Fact]
    public async Task EnumerateResourcesAsync_DomainResolverSet_CallsResolverForEachResource()
    {
        // Arrange
        var uris = new[]
        {
    ...
    public async Task EnumerateResourcesAsync_DomainResolverSet_ReturnsResourcesWithTheResolvedDomain()
    {
        ...
        _options.ManifestReaders = [CreateManifestReader(mediaTypes: ["x-reader/a"], urls: uris)];
        _options.DomainResolver = u => $"my-domain-{u.AbsolutePath[^1]}";
