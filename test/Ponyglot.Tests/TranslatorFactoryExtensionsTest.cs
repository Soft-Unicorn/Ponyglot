...
public class TranslatorFactoryExtensionsTest
{
    ...
    [Fact]
    public void CreateOfT_Always_CallsTheFactoryWithTheCorrectDomainAndContext()
    {
        // Arrange
        ...
        // Assert
        var domain = GetDomain(typeof(TranslatorFactoryExtensionsTest));
        var context = GetContext(typeof(TranslatorFactoryExtensionsTest));
        _factory.Received(1).Create(domain, context);
    }
    ...
    public void Create_ValidArguments_CallsTheFactoryWithTheCorrectDomainAndContext()
    {
        // Arrange
    ...
    private static string GetDomain(Type type) => type.Assembly.GetName().Name ?? throw new InvalidOperationException($"Assembly name is not available for type {type.FullName}.");
