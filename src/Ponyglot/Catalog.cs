...
public class Catalog
{
    ...
    /// <param name="domain">The domain or catalog name the translations belong to.</param>
    ...
    public Catalog(Uri uri, string domain, CultureInfo culture, IPluralRule pluralRule, IEnumerable<MessageEntry> entries)
    {
        ...
        {
            // Validate
            if (!ValidateEntry(entry, entryIndex++, pluralRule, uri, out var entryError))
            {
            ...
            {
                throw new ArgumentException($"Duplicate catalog entry for message '{entry.MessageId}' with context '{entry.Context}' in catalog '{uri}'.", nameof(entries));
            }
        }
    ...
    /// <param name="catalogUri">The URI of the catalog used to produce meaningful error messages.</param>
    /// <param name="errorMessage">When this method returns <c>false</c>, contains the error message; otherwise, <c>null</c>.</param>
    /// <returns><c>true</c> if the <paramref name="entry"/> is valid; otherwise, <c>false</c>.</returns>
    private static bool ValidateEntry(MessageEntry? entry, int index, IPluralRule pluralRule, Uri catalogUri, [NotNullWhen(false)] out string? errorMessage)
    {
        if (entry == null)
        {
        ...
        {
            errorMessage = $"No translations defined for message '{entry.MessageId}' with context '{entry.Context}' in catalog '{catalogUri}'.";
            return false;
        }
        ...
        if (entry.IsPlural && entry.Translations.Count != pluralRule.PluralCount)
        {
            ...
            errorMessage = $"Plurals count ({actualCount}) mismatch for message '{entry.MessageId}' with context '{entry.Context}' in catalog '{catalogUri}' (expected {expectedCount}).";
            return false;
        }
            ...
            if (entry.Translations[i] is { IsCompositeFormat: true } translation)
            {
                ...
                catch (FormatException exception)
                {
                    errorMessage = $"Invalid translation format string ({translation.Message}) " +
                                   $"at index {i} for message '{entry.MessageId}' with context '{entry.Context}' in catalog '{catalogUri}': " +
                                   $"{exception.Message}";
                    return false;
                }
            }
