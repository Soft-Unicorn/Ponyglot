...
public class Translator : ITranslator
{
    ...
    public string T(string messageId, params object[]? args)
    {
        ...
        try
        {
            translation = _translationStore.Find(Domain, _cultureSource.Culture, Context, messageId);
            var formattedTranslation = string.Format(_cultureSource.Culture, translation ?? messageId, args ?? []);
            return formattedTranslation;
        }
        catch (Exception exception)
        {
            throw BuildTranslationError(messageId, translation, args, exception);
        }
    }
    ...
    public string N(long count, string messageId, string pluralId, params object[]? args)
    {
        ...
        try
        {
            translation = _translationStore.FindPlural(Domain, _cultureSource.Culture, Context, count, messageId, pluralId);
            var formattedTranslation = string.Format(_cultureSource.Culture, translation ?? messageId, args ?? []);
            return formattedTranslation;
        }
        catch (Exception exception)
