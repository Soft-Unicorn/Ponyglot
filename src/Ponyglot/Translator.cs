...
public class Translator : ITranslator
{
    ...
    /// is <c>null</c>.
    /// </exception>
    public Translator(TranslationStore translationStore, ICultureSource cultureSource, string domain, string context)
    {
    ...
    /// <inheritdoc/>
    public ITranslator ForCulture(CultureInfo culture) => new Translator(Store, new FixedCultureSource(culture), Domain, Context);
    ...
    protected virtual string GetTranslation(long? count, string messageId, string defaultMessage, object?[]? args)
    {
        ...
        try
        {
            ...
            if (Store.TryGet(Domain, CultureSource.Culture, Context, count, messageId, out var foundTranslation))
            {
