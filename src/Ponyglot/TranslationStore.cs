...
public class TranslationStore
{
    ...
    /// <exception cref="ArgumentException"><paramref name="catalogs"/> contains more than one catalog for the same domain and culture or an invalid catalog.</exception>
    public virtual void Initialize(IEnumerable<Catalog> catalogs)
    {
        ...
        foreach (var catalog in catalogs)
        {
            ...
            {
                var otherUri = cultureIndex[catalog.Culture.Name].Uri;
                var currentUri = catalog.Uri;
                throw new ArgumentException(
                    $"The list of catalogs contains more than one catalog for domain '{catalog.Domain}' and culture '{catalog.Culture.Name}': '{otherUri}' and '{currentUri}'.",
                    nameof(catalogs));
            }
