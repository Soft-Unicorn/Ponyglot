...
{
    /// <summary>
    /// Creates a new <see cref="ITranslator"/> instance for <typeparamref name="T"/>, deriving the domain and context from the <see cref="Type"/>.
    /// </summary>
    ...
    /// <remarks>The domain and context are derived from the specified <typeparamref name="T"/> type using the <see cref="TranslatorConventions.ResolveType"/> method.</remarks>
    ...
    public static ITranslator Create<T>(this ITranslatorFactory factory)
    {
        ...
        var (domain, context) = TranslatorConventions.ResolveType(typeof(T));
        return factory.Create(domain, context);
    }

    /// <summary>
    /// Creates a new <see cref="ITranslator"/> instance for <paramref name="type"/>, deriving the domain and context from the <see cref="Type"/>.
    /// </summary>
    /// <param name="factory">The <see cref="ITranslatorFactory"/> to extend.</param>
    /// <param name="type">The <see cref="Type"/> to create a translator for.</param>
    /// <returns>A new <see cref="ITranslator"/> instance.</returns>
    /// <remarks>The domain and context are derived from the specified <paramref name="type"/> using the <see cref="TranslatorConventions.ResolveType"/> method.</remarks>
