...
public class AssemblyCatalogSourceOptions
{
    ...
    /// <summary>
    /// The optional resolver that can extract the domain name from the <see cref="System.Reflection.Assembly"/> and embedded resource name. Default is <c>null</c>.
    /// </summary>
    /// <remarks>
    /// When <c>null</c>, the default behavior is applied.
    /// <para>
    /// The default domain name extraction behavior is to use the assembly short name <see cref="AssemblyName.Name"/> (<c>assembly.GetName().Name)</c>.
    /// </para>
    /// </remarks>
    public Func<Assembly, string, string>? DomainResolver { get; set; }
}