...
public class HttpCatalogSource : StreamCatalogSource
{
    ...
    protected override async IAsyncEnumerable<StreamResource> EnumerateResourcesAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await using (responseStream.ConfigureAwait(false))
        {
            ...
            await foreach (var rawUri in reader.ReadAsync(responseStream, cancellationToken).ConfigureAwait(false))
            {
                ...
                var domain = domainResolver(uri);
                var uid = $"Http:Uri={uri}";
                yield return new HttpStreamResource(Client, uid, uri, domain, Options.ConfigureCatalogRequest);
            }
        }
    }
    ...
    /// <returns>The domain name for the specified catalog resource.</returns>
    private static string ResolveDomain(Uri uri)
    {
    ...
    /// </summary>
    private sealed class HttpStreamResource : StreamResource
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="HttpStreamResource"/> class.
        /// </summary>
        /// <param name="httpClient">The <see cref="HttpClient"/> to use to perform http requests.</param>
        /// <param name="uid">An identifier that uniquely identifies the resource.</param>
        /// <param name="uri">The <see cref="Uri"/> of the catalog resource.</param>
        /// <param name="domain">The default domain or catalog name to use if this information cannot be derived from the resource data.</param>
        /// <param name="configureRequest">The optional action that can be used to configure the <see cref="HttpRequestMessage"/> used to fetch a catalog resource.</param>
        public HttpStreamResource(HttpClient httpClient, string uid, Uri uri, string domain, Action<HttpRequestMessage>? configureRequest)
            : base(uid, uri.ToString(), domain)
        {
