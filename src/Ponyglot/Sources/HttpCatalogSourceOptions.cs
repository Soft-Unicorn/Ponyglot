...
public class HttpCatalogSourceOptions
{
    ...
    /// <summary>
    /// The optional resolver that can extract the domain name from the <see cref="Uri"/> of the catalog resource. Default is <c>null</c>.
    /// </summary>
    /// <remarks>
    /// When <c>null</c>, the default behavior is applied.
    /// <para>
    /// The default domain name extraction behavior uses the last URI path segment (<c>uri.Segments[^1]</c>) as the file name.
    /// Then the part of the file name before the first dot or the file name if it contains no dot (the initial dot is skipped) is used as the domain name.
    /// </para>
    ...
    /// </list>
    /// </remarks>
    public Func<Uri, string>? DomainResolver { get; set; }
